<?php session_start();
    ...
    if(isset($_POST['CON_47'], $_GET['sending_by'])){
        ...
        if(isset($user_id)){
            if(mysqli_num_rows($rqt_std = mysqli_query($db, "SELECT * FROM students WHERE students.id = $user_id AND students.delete = 0 AND students.active = 1 AND students.valide = 1 ORDER BY firstname"))){
                    ...
                    $rqtCotes = mysqli_query($db,
                            "SELECT courses.name, level.result_passed AS lev_passed, level.terminal AS lev_terminal, courses.credit, cotes.*, classes.id AS class
                                    FROM courses 
                                        ...
                                        JOIN students ON students.id = $user_id
                ...
                $res_exist_delib = mysqli_fetch_array(mysqli_query($db, "SELECT * FROM deliberation WHERE `student` = '$user_id' AND `class` = '$class' AND `session` = YEAR(CURRENT_TIMESTAMP)"));
                
                $updated = " `deliberation` SET `admin` = '$user->id', `student` = '$user_id', `class` = '$class', `result` = '$COTES_FINALE', `ended` = '$finished', `valide` = '$dte', `datatime` = CURRENT_TIMESTAMP ";
                ...
                mysqli_query($db, "UPDATE `students` SET `passed` = '$passed', `finished` = '$finished' WHERE `students`.`id` = $user_id;") or die(json_encode(array(
                        "status" => "error",
                        "code" => "5017",
                        "content" => "Error SQL : ".mysqli_error($db)
                    )));
