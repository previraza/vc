    ...
    <div class="row"> 
        ...
        <div class="col-lg-8">
            ...
            <?php foreach ($Fee->allMine('*') AS $fee) {
                    ...
                    $devise = $fee->devise;
                    $transh = $fee->transh??$fee->value;
                    $Account->$devise->payment->count += $fee->payment;
                    $Account->$devise->slip->count += $fee->slip;
                    $fee->full  = $payment_solde += $fee->value * $fee->payment;
                    $fee->full += $slip_solde += $fee->value * $fee->slip;
                    if($transh&&$transh!='full'){
                        ...
                        $fee->first     = filterValue($transh, 'first') * $fee->payment_first;

                        $payment_solde += filterValue($transh, 'second') * $fee->payment_second;
                        $fee->second    = filterValue($transh, 'second') * $fee->payment_second;
                        
                        $slip_solde += filterValue($transh, 'first') * $fee->slip_first;
                        $fee->first += filterValue($transh, 'first') * $fee->slip_first;
                        
                        $slip_solde += filterValue($transh, 'second') * $fee->slip_second;
                        $fee->second+= filterValue($transh, 'second') * $fee->slip_second;
                    }
                    
                    $Account->$devise->payment->solde += $payment_solde;
                    $Account->$devise->slip->solde += $slip_solde;
                    $fee->solde = ($fee->first??0)+($fee->second??0)+($fee->full??0);
                    $fee->registration = ($fee->payment+=$fee->payment_first+$fee->payment_second) + ($fee->slip+=$fee->slip_first+$fee->slip_second);
                    $FilterFee[]=$fee; $fee->transh = $transh??$fee->value;
                ?>
                        ...
                        <div class="col-md-12">
                            <div class="overflow-hidden card" data-aos="fade-up" data-aos-delay="600">
                                ...
                                <div class="px-0 card-body">
                                    <?=HTML::createTable(
                                        ['Libellé', 'Montant', 'Tranche', 'Dévise', 'Paeiments par tranches' => ['Première', 'Deuxième', 'Complète'], 'Enregistrement' => ['P', 'B', 'T'], 'Somme', 'Code', 'Autorisation'],
                                        array_merge($FilterFee ?? []),
                                        ['name', 'value', 'transh', 'devise', 'first', 'second', 'full', 'payment', 'slip', 'registration', 'solde', 'code', 'allower']
                                    )?>
                                </div>
                            </div>
                        </div>
