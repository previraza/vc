                        ...
                        <form class="row align-items-end justify-content-between" action=<?= $data_set ? str_replace('edit', 'update', url()) : root($this->request->space.'/add') ?> method=POST dynamic>
                            ...
                            <div class="form-contenair">
                                <div class="row">
                                    ...
                                    <?php } else { ?>
                                        <div class="col-lg-8">
                                            <?=Form::input('client_name', 'Nom d\'Etudiants')?>
                                        </div>
                                        <div class="col-lg-4">
                                            <?=Form::select('gender', [
                                                'M'=>'Masculin',
                                        ...
                                        <div class="col-lg-8">
                                            <?=Form::select('client_class', $classes ?? [], 'Classes')?>
                                        </div>
                                        <div class="col-lg-4">
                                            <?=Form::input('contact', 'Numéro Matricule', false)?>
                                        </div>
                                    <?php } ?>
                                    <div class="col-lg-4">
                                        <?=Form::select('fee', loadModel('fees'), 'Frais')?>
                                    </div>
                                    <div class="col-lg-4">
                                        <?=Form::selectMultiple('transhis', [
                                            'first'     =>  '1. Premiere Tranche',
                                            'second'    =>  '2. Deuxieme Tranche',
                                        ], 'Tranche', false)?>
                                    </div>
                                    <div class="col-lg-4">
                                        <?=Form::input('number', 'Numero de bordeaux')?>
                                    </div>
                                </div>
                            </div>
                            <div class="text-center mt-4">
                                <button type="save" class="btn btn-primary" name=FORM::PAYMENT
                                    value='<?= $data_set ? 'UPDATE' : 'INSERT' ?>'>Confirmer</button>
                            </div>
                        </form>
                    </div>
                </div>
