<?php 
    ...
    $fc = $db->query(
        "SELECT
            financy_payment.id, financy_payment.ref, financy_payment.contact, financy_payment.created_at, financy_payment.code, financy_payment.transhis, financy_fee.value, financy_fee.value AS full_value, financy_fee.transh, financy_fee.devise,
            users_financy.fullname                     AS agent,
            financy_payment.client_name                AS name,
            CONCAT(std1.firstname, ' ', std1.lastname) AS name1,
            CONCAT(std2.firstname, ' ', std2.lastname) AS name2,
            financy_payment.contact                    AS phone,
            ...
            c1.name                                    AS cla1,
            c2.name                                    AS cla2,
            DATE(financy_payment.created_at)           AS today, 
            MONTH(financy_payment.created_at)          AS month, 
            financy_fee.name                           AS fee,
            financy_fee.code                           AS fee_code
        FROM financy_payment 
            JOIN      financy_fee           ON financy_fee.id=financy_payment.fee
            JOIN      users_financy         ON users_financy.id=financy_payment.created_by
            LEFT JOIN snem.students AS std1 ON std1.id=financy_payment.student 
            LEFT JOIN snem.students AS std2 ON std2.id=financy_payment.student
        ...
        WHERE financy_payment.id = $code AND financy_payment.deleted_at IS NULL ORDER BY id") or die($db->error);
    $db->query("UPDATE financy_payment SET printed = true WHERE financy_payment.id = $code;");
    $fac = $fc->fetch_object();
    if (!empty($fac)){ $f = $fac;
