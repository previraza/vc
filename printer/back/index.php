<?php 
    ...
    $fc = $db_poo->query("SELECT payments.*, register.telephone, register.sexe, CONCAT(agents.firstname, ' ', agents.lastname) AS agent, CONCAT(register.nom, ' ', register.postnom) AS std, frais.name AS type_frais 
        FROM payments LEFT JOIN frais ON payments.type = frais.type JOIN agents ON payments.user_id = agents.user_id LEFT JOIN register ON register.id = payments.student  WHERE payments.id = $_GET[id]") or die($db_poo->error);
    $fac = $fc->fetch_object();
    if (!empty($fac)){ $f = $fac;
    ...
    $FRAIS=[]; $rqt_00 = $db_poo->query("SELECT type, name FROM frais WHERE frais.active = 1 AND frais.delete = 0");
    while($res = $rqt_00->fetch_object()) $FRAIS[$res->type] = $res->name ;
    function getFrais($tof) : String {
        $FRAIS = $GLOBALS['FRAIS'];
        return isset($FRAIS, $FRAIS[$tof]) ? $FRAIS[$tof] : "<i>Undefined</i>";
    }
?>
<!doctype html><html class="no-js" lang="zxx">
    <head><meta charset="utf-8"><meta http-equiv="x-ua-compatible"
            content="ie=edge"><title>FACTURE INBTP KINSHASA NGALIEMA - <?= $f->student ?> - <?=$f->std?$f->std:$f->student_name?></title><meta name="author" content="Angfuzsoft"><meta
            name="description" content="Invce - Invoice HTML Template"><meta
            name="keywords" content="Invce - Invoice HTML Template"><meta
            ...
            href="assets/css/style.css"></head><body>
                <div
            class="invoice-container-wrap"><div class="invoice-container"><main><div
                        class="as-invoice invoice_style2"><div
                            class="download-inner" id="download_section"><header
                                class="as-header header-layout1"><div class="row
                                    align-items-center justify-content-between"><div
                                        class="col-auto"><p class="mb-0"><b>INVOICE
                                                NO: #<?= $f->number?></b></p></div><div
                                        class="col-auto"><div
                                            class="header-logo"><a
                                                href="index.html"><img style="max-height:50px;"
                                                    src="/public/assets/imgs/home26.png"
                                                    alt="Invce"></a></div></div></div><div
                                    class="header-bottom"><div class="row
                                        align-items-center
                                        justify-content-between"><div
                                            class="col-auto"><div
                                                class="header-bottom_left"><p><b>Nom de l'agent : </b><?= $f->agent?></p><div
                                                    class="shape"></div><div
                                                    class="shape"></div></div></div><div
                                            class="col-auto"><div
                                                class="header-bottom_right"><div
                                                    class="shape"></div><div
                                                    class="shape"></div><p><b>Date:
                                                    </b><?= $f->date?></p></div></div></div></div>
                                                    </header>
                                                    <div
                                class="row justify-content-between">
                                                        <div
                                    class="col-auto"><div class="booking-info"><p><b>Institut National du Bâtiment et des Travaux Publics - INBTP Kinshasa
                                            </b></p><p><b>Mode de Payement :
                                            </b>
                                            <?= ["M"=>"Perception Manuelle", "B"=>"Bank Boa", "R"=>"Bank Rawbank"][$f->bank] ?>    
                                            </p></div></div></div>
                                            <div
                                class="row gx-0"><div class="col-6"><div
                                        class="address-box address-left"><b>Vos
                                            Informations :</b><address><?=$f->std?$f->std:$f->student_name?><br>Phone: <?=$f->telephone?><br>Sexe:
                                            <?= ["_"=>"","A"=>"Autre","F"=>"Féminin","M"=>"Masculin"][$f->sexe?$f->sexe:'_']?><br></address></div></div><div
                                    class="col-6"><div class="address-box
                                        address-right"><b>Details Agences:</b><address>Vira-Bank<br>Guichet No. 1<br>Kinshasa-Ngaliema,
                                            R.D.Congo.</address></div></div></div><table
                                class="invoice-table"><thead><tr><th>Déscription</th><th>Montant</th></tr></thead>
                                <tbody><tr>
                                    <td><?= $f->type_frais ?? getFrais($f->type) ?></td><td><?=write_sum($f->sum,$f->devise)?></td></tr>
                                    <tr><td
                                            colspan="3">&nbsp;</td></tr></tbody></table><div
                                class="row justify-content-between"><div
                                    class="col-auto"><div class="invoice-left"><b>Please
                                            Note:</b><p class="mb-0">Le montant payé ne peut etre ni repris, ni transférer à un tiers.</p></div></div><div
                                    class="col-auto"><table class="total-table"><tr><th>Sub
                                                Total:</th><td><?=write_sum($f->sum,$f->devise)?></td></tr><tr><th>Tax:</th><td><?=write_sum(0,$f->devise)?></td></tr><tr><th>Total:</th><td><?=write_sum($f->sum,$f->devise)?></td></tr></table></div></div><p
                                class="company-address"><b>Invce Inc:</b><br>21, Av. de la Montagne, Q. Joli-Parc, Kinshasa-Ngaliema, République Démocratique du Congo</p><p
                                class="invoice-note mt-3 text-center"><svg
                                    ...
                                    xmlns="http://www.w3.org/2000/svg"><path
                                        ...
                                        1.2V4.92V1.2V14.8V1.2Z" fill="#1CB9C8"/></svg>
                                    <b>NOTE: </b>La facture ci-dessus a été générée par l'ordinateur de la 
                                     réception et ne nécessite pas de Signature.</p></div><div
                                class="invoice-buttons"><button
                                    class="print_btn"><svg width="16"
                                        height="16" viewBox="0 0 16 16"
                                        fill="none"
                                        xmlns="http://www.w3.org/2000/svg"><path
                                            d="M11.9688 8.46875C12.1146 8.32292
                                            ...
                                            7.72917 1.5 8V11H14.5Z"
                                            fill="white"/></svg> <span>Print</span></button>
                                    <button id="download_btn"
                                        class="download_btn"><svg width="18"
                                            height="16" viewBox="0 0 18 16"
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"><path
                                                d="M16.5 9C16.9167 9 17.2708
                                                ...
                                                13.0312Z" fill="white"/></svg>
                                            <span>Download</span></button></div></div></main></div></div><script
